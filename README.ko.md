...
# XXD Panel 166｜등각 투영 조각적 부조 디오라마
...
원본 출력 PNG를 유지했으며 크기 조정, 자르기, 분할선 보정을 하지 않았습니다. 가로는 1672×941(약 16:9), 세로는 1086×1448(3:4)입니다. 가로 분할선은 중앙 부근이며 세로 원본 영역은 약 44.3–50.1%입니다. 모두 정확한 50:50 분할은 아닙니다.
...
등각 투영의 압축된 조각적 부피와 계단 지형, 얕은 부조입니다. 2–4색을 쓰며 사실적 CG나 게임 에셋은 피합니다.
...
- `design-only`: 전체 캔버스에 Panel 166의 디자인 번역만 표시하고 사진은 보이지 않는 참고 자료로 사용합니다.
...
설치 후 Agent 세션을 다시 시작하고 `$xxd-panel-166`을 호출하세요. 사용자 단위 Codex 설치에는 `--global --agent codex --yes`를 추가할 수 있습니다.

```text
/xxd-panel-166 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ko-KR
...
전체 실행 계약은 [SKILL.md](SKILL.md), 런타임 어댑터는 [영어](references/xxd-panel-166-prompt.en.md)와 [중국어](references/xxd-panel-166-prompt.zh-CN.md)를 확인하세요.
