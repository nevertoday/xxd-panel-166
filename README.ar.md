...
# XXD Panel 166｜ديوراما متساوية القياس بنحت بارز ضحل
...
ثمانية نماذج أولية من Panel 166، وُلّد كل منها مستقلاً من صورة مصدر مختلفة مع نص إنجليزي قصير. الأفقي 1536×864 بنصفين متساويين يميناً ويساراً؛ والعمودي 1152×1536 بنصفين متساويين أعلى وأسفل.

رُوجعت صلة الصور بالمصادر والمنصات متساوية القياس وطبقات النحت البارز. تحتفظ بعض النتائج بمشاهد مفرطة وتفاصيل واقعية ومجسمات كبيرة؛ النموذج 10 لامع، و05 و09 يضيفان عينات ألوان. هذه نتائج أولية فعلية وليست كلها مطابقة لمتطلبات تبسيط الكتل والألوان المحدودة والفراغ الواسع.
...
كتل نحتية مضغوطة متساوية القياس وتضاريس متدرجة ونحت ضحل، بلوحة 2–4 ألوان، بعيداً عن واقعية CG وأصول الألعاب.
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 166 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-166`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-166 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-166-prompt.en.md) أو [بالصينية](references/xxd-panel-166-prompt.zh-CN.md).
